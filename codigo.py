...
SENTINEL_BANDS_2020 = {
    'B02': r"E:\mestrado\mest3\2020\R10m\T24KVG_20201007T125311_B02_10m.jp2",
    'B03': r"E:\mestrado\mest3\2020\R10m\T24KVG_20201007T125311_B03_10m.jp2",
    'B04': r"E:\mestrado\mest3\2020\R10m\T24KVG_20201007T125311_B04_10m.jp2",
    'B08': r"E:\mestrado\mest3\2020\R10m\T24KVG_20201007T125311_B08_10m.jp2"
}
SENTINEL_BANDS_2023 = {
    'B02': r"E:\mestrado\mest3\2023\R10m\T24KVG_20230927T125309_B02_10m.jp2",
    'B03': r"E:\mestrado\mest3\2023\R10m\T24KVG_20230927T125309_B03_10m.jp2",
    'B04': r"E:\mestrado\mest3\2023\R10m\T24KVG_20230927T125309_B04_10m.jp2",
    'B08': r"E:\mestrado\mest3\2023\R10m\T24KVG_20230927T125309_B08_10m.jp2"
}

STUDY_AREA_SHP = r"E:\mestrado\mest3\limite\limite.shp"
OUTPUT_DIR_BASE = r'E:\mestrado\mest3\resultados_comparativos_filtrado'
GROUND_TRUTH_SHP = r'E:\mestrado\mest3\verdade de campo\pontos.shp'
...
def coletar_dados_treinamento_combinado(anos_para_processar, shape_limite_path, shp_ground_truth_path, col_classe_gt):
    ...
    # 1. Ler dados vetoriais (limite e pontos de referência) uma única vez
    ...
    # Usar o primeiro ano como referência para CRS e geometria
    ...
    # 2. Processar os pontos GT para obter coordenadas e classes
    ...
    # 3. Iterar sobre cada ano para extrair os valores dos pixels (features)
    for ano_str, bandas_paths in anos_para_processar.items():
        ...
        if not y_all:  # Adiciona os labels na primeira iteração válida
            y_all.extend(y_ano_list)
        else: # Nas iterações seguintes, apenas confirma se a quantidade é a mesma.
             y_all.extend(y_ano_list)
...
def treinar_e_validar_modelo(X_all, y_all, metodo_str, gdf_gt_usado):
    """
    Treina e valida (usando K-Fold) um modelo com o dataset combinado.
    """
    ...
    X_all_final_train = X_all.copy()
    scaler_final = None
    if metodo_str_upper == 'SVM' and SVM_APPLY_SCALING:
        scaler_final = StandardScaler().fit(X_all_final_train)
        X_all_final_train = scaler_final.transform(X_all_final_train)
    ...
    model_final.fit(X_all_final_train, y_all)
...
def aplicar_modelo_e_exportar_resultados(ano_str, metodo_str, bandas_paths, shape_limite_path, model, output_dir_metodo):
    ...
    try:
        # 1. Leitura dos dados geoespaciais do ano específico
        limite_gdf = gpd.read_file(shape_limite_path)
        ...
        # 2. Predição e Pós-processamento
        ...
        if hasattr(model, 'scaler') and model.scaler is not None:
            X_pred_map = model.scaler.transform(X_pred_map)
        ...
        if coords_para_shape:
            shapes_path, centroid_path = exportar_shapes_finais(
                transform_geo=out_profile_final['transform'], crs_geo=out_profile_final['crs'],
                todos_grupos_coords=coords_para_shape, output_dir=output_dir_metodo, sufixo_ano_metodo=metodo_id)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ETAPA 1: Coletar todos os dados de treinamento de uma só vez
        X_combinado, y_combinado, gdf_gt_completo = coletar_dados_treinamento_combinado(
            anos_para_processar, STUDY_AREA_SHP, GROUND_TRUTH_SHP, COLUNA_CLASSE_REF
        )

        
        # ETAPA 2: Loop por MÉTODO para treinar e depois aplicar em cada ano
        for metodo in METHODS_TO_PROCESS:
            ...
            # 2a. Treinar e validar o modelo UMA VEZ com os dados combinados
            modelo_final, metricas = treinar_e_validar_modelo(X_combinado, y_combinado, metodo, gdf_gt_completo)

            # Armazenar métricas e escrever relatório de validação para o método
            if metricas:
                ...
                escrever_relatorio_validacao(
                    metodo_id=metodo.lower(), 
                    anos_str=', '.join(anos_para_processar.keys()),
                    metodo_str=metodo, 
                    shp_gt_path=GROUND_TRUTH_SHP,
                    dir_output_valid=output_dir_valid, 
                    col_classe_gt=COLUNA_CLASSE_REF, 
                    metricas=metricas, 
                    gdf_gt_usado=gdf_gt_completo
                )

            # 2b. Loop por ANO para APLICAR o modelo treinado e exportar resultados
            for ano, bandas in anos_para_processar.items():
                run_id = f"{ano}_{metodo.lower()}"
                raster_array, raster_profile = aplicar_modelo_e_exportar_resultados(
                    ano, metodo, bandas, STUDY_AREA_SHP, modelo_final, output_dir_atual
                )
    ...
    # ETAPA 3: Análises Finais (Comparação e Interseção) 
